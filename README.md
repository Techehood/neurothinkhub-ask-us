...
- Minimal feedback is sent server-to-server; conversation text requires explicit consent.
...
| `FEEDBACK_WEBHOOK_URL`    | Yes for feedback       | Server-only HTTPS destination for feedback events.                                    |
| `FEEDBACK_WEBHOOK_TOKEN`  | Depends on destination | Server-only bearer credential for the feedback destination.                           |
| `FEEDBACK_RETENTION_DAYS` | No                     | Adds a deletion deadline to each feedback event.                                      |
